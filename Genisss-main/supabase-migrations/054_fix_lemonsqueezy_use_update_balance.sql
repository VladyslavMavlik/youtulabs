-- Fix LemonSqueezy functions to use update_user_balance instead of direct INSERT/UPDATE
-- Also add test mode variant IDs
...
-- Add 'subscription' type to balance_transactions if not exists
DO $$
BEGIN
  -- Drop and recreate the constraint to include 'subscription' type
  ...
  ALTER TABLE balance_transactions ADD CONSTRAINT balance_transactions_type_check
    CHECK (type IN ('purchase', 'generation', 'refund', 'bonus', 'initial', 'subscription', 'crypto'));
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Could not update constraint: %', SQLERRM;
END;
$$;

-- Fix process_lemonsqueezy_subscription_created to use update_user_balance
...
BEGIN
  ...
  v_balance_before := COALESCE(get_user_balance(p_user_id), 0);

  -- Use update_user_balance function to add credits properly
  SELECT new_balance, transaction_id INTO v_balance_after, v_transaction_id
  FROM update_user_balance(
    p_user_id,
    v_credits,
    'subscription',
...
BEGIN
  ...
  -- Get current balance
  v_balance_before := COALESCE(get_user_balance(v_sub.user_id), 0);

  -- Use update_user_balance for renewal credits
  SELECT new_balance, transaction_id INTO v_balance_after, v_transaction_id
  FROM update_user_balance(
    v_sub.user_id,
